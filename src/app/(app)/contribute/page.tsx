...
import { MapPin, FileText, Image as ImageIcon, Video, Link as LinkIcon, Plus, Trash2, ArrowLeft, CheckCircle2, Loader2, Youtube } from "lucide-react";
	...
	if (success) {
		return (
			<div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
				...
				<p className="text-muted-foreground text-center max-w-md">Your contribution has been submitted. Redirecting to the activity feed...</p>
			</div>
		);
	}

	return (
		<div className="max-w-2xl mx-auto space-y-6 p-3 pb-20 sm:p-4 md:p-6 md:pb-6">
			{/* Header */}
			<div className="flex items-center gap-3">
				...
				<div>
					...
					<p className="text-sm text-muted-foreground">Share research, media, or add new sites to the map</p>
				</div>
			</div>
				...
				<TabsContent value="post" className="mt-4">
					<Card className="border-border/40 bg-card">
						<CardHeader>
							...
							<CardDescription>Share research findings, link YouTube videos, add images, or start a discussion</CardDescription>
						</CardHeader>
						<CardContent>
							<form onSubmit={handleSubmitPost} className="space-y-4">
								...
								<div className="space-y-2">
									...
									<Textarea
										...
										placeholder="Share your research, thoughts, or findings...

Use #hashtags to tag topics (e.g., #gobekli-tepe #precision-engineering)"
										rows={6}
										className="bg-background/50"
										required
									/>
									<p className="text-xs text-muted-foreground flex items-center gap-1">
										...
										Use #hashtags to link your post to topics
									</p>
									{/* Show detected hashtags */}
									{extractHashtags(postBody).length > 0 && (
										<div className="flex items-center gap-2 flex-wrap pt-1">
											<span className="text-xs text-muted-foreground">Tags:</span>
											{extractHashtags(postBody).map((tag) => (
									...
									{externalLinks.length > 0 && (
										<div className="space-y-2">
											{externalLinks.map((link) => (
												<div key={link.id} className="flex items-center gap-2 p-2 rounded-lg bg-background/30 text-sm">
													{link.type === "youtube" ? (
													...
													<Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => removeLink(link.id)}>
				...
				<TabsContent value="site" className="mt-4">
					<Card className="border-border/40 bg-card">
						<CardHeader>
							...
							<CardDescription>Add a megalithic or ancient site to the map. Sites go through community review before being verified.</CardDescription>
						</CardHeader>
