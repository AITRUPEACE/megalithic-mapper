...
export const SiteDetailPanel = ({ site, className, variant = "card" }: SiteDetailPanelProps) => {
	if (!site) {
		...
		return (
			<div className={cn(emptyClasses, className)}>Select a site marker or list item to view research connections, metadata, and next steps.</div>
		);
	}
		...
		<div className="flex flex-wrap items-center justify-between gap-3">
			<div>
				...
				<p className="text-xs uppercase tracking-wide text-muted-foreground">
					{site.tags.cultures[0] || "Unknown culture"} - {site.tags.eras[0] || "Unknown era"}
				</p>
			</div>
		...
		<Tabs defaultValue="overview" className="flex h-full flex-col gap-4">
					...
					<div className="space-y-2 text-xs">
						...
						<div className="flex flex-wrap gap-2">
							{site.zoneMemberships.map((zone) => (
								<span key={zone.id} className="rounded-full px-3 py-1" style={{ backgroundColor: `${zone.color}22`, color: zone.color }}>
									{zone.name}
								</span>
							))}
						</div>
			...
			<TabsContent value="activity" className="flex-1 overflow-y-auto space-y-3 text-sm text-muted-foreground">
				...
				<ul className="space-y-2 text-xs">
					<li>
						<span className="font-semibold text-foreground">Last updated:</span> {timeAgo(site.updatedAt)} by {site.updatedBy}
					</li>
