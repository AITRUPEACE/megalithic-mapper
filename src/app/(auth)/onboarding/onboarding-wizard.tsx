...
const stepSchemas = [
	onboardingSchema.pick({ username: true, fullName: true, headline: true, location: true, defaultViewport: true }),
	...
	const handleSubmit = () => {
		...
		startTransition(async () => {
			try {
				...
				console.error(error);
				setServerError(error instanceof Error ? error.message : "We could not save your onboarding data.");
			}
		});
	};
					...
					<div className="space-y-4">
						<div>
							...
							<Input
								...
								placeholder="indiana_jones"
							/>
							{fieldErrors.username && <p className="text-sm text-destructive">{fieldErrors.username}</p>}
						</div>
						<div>
							<label className="text-sm font-medium">Full name</label>
							<Input value={formValues.fullName} onChange={(event) => updateField("fullName", event.target.value)} placeholder="Dr. Aiyana Reyes" />
						...
						<div>
							...
							/>
							{fieldErrors.headline && <p className="text-sm text-destructive">{fieldErrors.headline}</p>}
						</div>
						<div>
							<label className="text-sm font-medium">Location</label>
							<Input
								value={formValues.location ?? ""}
								onChange={(event) => updateField("location", event.target.value.trim() ? event.target.value : null)}
						...
						<div className="space-y-2">
							...
							{fieldErrors["defaultViewport"] && <p className="text-sm text-destructive">{fieldErrors.defaultViewport}</p>}
						</div>
					</div>
				);
			case 1:
					...
					<div className="space-y-4">
						<div>
							...
							<div className="flex gap-2">
								<Input value={tagInput} onChange={(event) => setTagInput(event.target.value)} placeholder="e.g. archaeoastronomy" />
								<Button type="button" onClick={handleAddTag} variant="secondary">
									Add
								</Button>
							</div>
							{fieldErrors.expertiseTags && <p className="text-sm text-destructive">{fieldErrors.expertiseTags}</p>}
							<div className="flex flex-wrap gap-2 pt-3">
								{formValues.expertiseTags.length === 0 && <p className="text-sm text-muted-foreground">No tags added yet.</p>}
				...
				return (
					<div className="space-y-4">
						<div>
							...
							/>
							{fieldErrors.contributionIntent && <p className="text-sm text-destructive">{fieldErrors.contributionIntent}</p>}
						</div>
						<div>
							<label className="text-sm font-medium">Collaboration focus (optional)</label>
							<Textarea
								value={formValues.collaborationFocus ?? ""}
								onChange={(event) => updateField("collaborationFocus", event.target.value.trim() ? event.target.value : null)}
								rows={3}
								placeholder="List the types of partners, digs, or comparative research you hope to join."
							/>
							{fieldErrors.collaborationFocus && <p className="text-sm text-destructive">{fieldErrors.collaborationFocus}</p>}
						</div>
					</div>
				);
			case 3:
						...
						<div className="rounded-lg border border-border/60 bg-background/40 p-4">
							<label className="flex items-start gap-3 text-sm">
								...
								<div>
									...
									<p className="text-muted-foreground">Weekly digests of dig logs, hypothesis debates, and map edits near your viewport.</p>
								</div>
							</label>
						</div>
						<div className="rounded-lg border border-border/60 bg-background/40 p-4">
							<label className="flex items-start gap-3 text-sm">
								...
								<div>
									...
									<p className="text-muted-foreground">Occasional updates about new map layers, verification tooling, and workshops.</p>
								</div>
							</label>
						</div>
					</div>
		...
		<div className="grid gap-8 lg:grid-cols-[2fr,1fr]">
			<div className="space-y-6">
				<div className="space-y-3">
					...
					<p className="text-muted-foreground">These details personalize map recommendations, collaboration invites, and verification prep.</p>
				</div>
				<Card className="border-border/40 bg-background/60">
					...
					<CardContent className="space-y-6">
							...
							{steps.map((step, index) => (
								<div
									...
									className={cn(
										...
										index === currentStep ? "border-primary bg-primary/10 text-primary" : "border-border/60 text-muted-foreground"
									)}
								>
						...
						<div className="flex flex-wrap items-center justify-between gap-3">
							<div className="text-xs text-muted-foreground">Progress saves automatically once you submit the final step.</div>
			...
			<div className="space-y-4">
				...
				<Card className="border-border/40 bg-background/40">
					<CardHeader>
						...
						<CardDescription>Your preferred map viewport and collaboration cues personalize the in-app feed immediately.</CardDescription>
					</CardHeader>
